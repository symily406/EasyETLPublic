		...
		<div class="form-container">
			<el-form ref="form" :model="form" :rules="rules">
				<el-form-item  prop="name"
					:rules="[{required: true, message: '请输入文件夹名称', trigger: 'blur'},{min:1,max:32,message:'文件夹名称长度不能超过32个字符'}]">
					<el-input v-model="form.name" placeholder="文件夹" size="small" maxlength="32"></el-input>
				</el-form-item>
			</el-form>
		</div>
		...
		methods: {
			...
			onSubmit(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.loading = true;
						this.$ajax.post(this.getContentPath() + "/admin/system/file/userResources/saveOrUpdate", this.form).then(
							res => {
								this.loading = false;
								const eventType = this.form.id == '' ? 10 : 20;
