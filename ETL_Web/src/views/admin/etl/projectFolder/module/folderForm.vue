			...
			<div class="form-container" v-loading="loading">
				<el-form ref="form" :model="form" label-width="120px" :rules="rules">
					<el-form-item label="目录名称" prop="folderName"
						...
						<el-input v-model="form.folderName" size="small" maxlength="64" placeholder="目录名称"></el-input>
					</el-form-item>
					<el-form-item label="目录位置" prop="taskFolderPid"
						:rules="[{required: true, message: '请选择目录位置', trigger: 'blur'}]" class="treeSelect"
						v-if="!form.taskFolderId">
						<treeSelect ref="treeSelect" v-model="form.taskFolderPid" :options="folderTreeData"
							:normalizer="normalizer" :clearable="false" placeholder="目录位置"
							:flatten-search-results="true" size="small" :filterable="true" @open="handleOpen"
							@select="hamdleSelect" :load-options="loadOptionsMethod" :appendToBody="true"
							noChildrenText="没有子选项" noOptionsText="没有可选项" noResultsText="没有匹配的结果" z-index="2005"  >
		...
		methods: {
			openWin() {
				this.$store.dispatch(
					"etl/task/initProjectTree", {
						...
						"noTaskFolderId": this.form.taskFolderid,
						"showFolderOnly": 1
					}).then(
					res => {
			...
			onSubmit(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.$ajax.post(this.getContentPath() + "/admin/etl/projectTaskFolder/saveOrUpdate", this
							.form).then(
							res => {
								this.loading = false;
								const eventType = this.form.taskFolderId == '' ? 1 : 2;
