<template>
	<div class="app-container" ref="appContainer">
		...
		<el-table size="mini" border :data="tableData" :height="tableHeight"
			...
			v-loading="loading">
			<el-table-column prop="workSpaceLayerName" label="名称">
				<template slot-scope="scope">
					{{scope.row.workSpaceLayerName}}
				</template>
			</el-table-column>
			...
			<el-table-column fixed="right" label="操作" width="150" align="center">
				<template slot-scope="scope">
					<el-button @click.native.prevent="editRow(scope.$index)" type="text" size="small"
						v-if="checkPermission('admin:etl:projectWorkSpaceLayer:edit')&&scope.row.allowEdit==1">
						修改
					</el-button>
					<el-button @click.native.prevent="deleteRow(scope.$index)" type="text" size="small"
						v-if="checkPermission('admin:etl:projectWorkSpaceLayer:deleteById')&&scope.row.allowDelete==1">
						删除
					</el-button>
				</template>
			</el-table-column>
		</el-table>
		<eForm ref="eForm" dialogTitle="添加分层" dialogWidth="800px" dialogHeight="350px"
			:dialogTop="this.$store.state.global.dialogTop" @eventCallBack="eventCallBack" />
		<edit ref="edit" dialogTitle="修改分层" dialogWidth="800px" dialogHeight="350px"
			:dialogTop="this.$store.state.global.dialogTop" @eventCallBack="eventCallBack" />
	</div>
</template>
<script>
	...
	export default {
		...
		methods: {
			initData(queryParams) {
				this.loading = true;
				this.$ajax.get(this.getContentPath() + "/admin/etl/projectWorkSpaceLayer/queryAll", queryParams).then(res => {
					this.loading = false;
			...
			deleteRow(index) {
				...
				if (row.allowDelete == 0) {
					this.$message.error('当前项目分层不能删除!');
					return false;
				}
				this.$confirm('您确定是否删除一行项目分层吗?<br>该操作不可恢复!!', '提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning',
					dangerouslyUseHTMLString: true
				}).then(() => {
					this.$ajax.get(this.getContentPath() + "/admin/etl/projectWorkSpaceLayer/deleteById", { "layerId": row
							.layerId }).then(
						res => {
