		...
		<el-scrollbar wrap-class="scrollbar-wrapper" :style="{height:dialogHeight}">
			<div class="form-container" v-loading="loading">
				<el-form ref="form" :model="form" label-width="120px" :rules="rules">
					...
					<el-form-item label="显示名称" prop="showMenuName"
						:rules="[{required: true, message: '请输入显示名称', trigger: 'blur'},{min:1,max:32,message:'显示名称长度不能超过32个字符'}]">
						<el-input v-model="form.showMenuName" placeholder="请输入显示名称" size="small" maxlength="32">
						</el-input>
					</el-form-item>
					<el-form-item label="标题" prop="title"
						:rules="[{required: false, message: '请输入标题', trigger: 'blur'},{min:1,max:32,message:'标题长度不能超过32个字符'}]"
						v-show="false">
						<el-input v-model="form.title" placeholder="请输入标题" size="small" maxlength="32"></el-input>
					</el-form-item>

					<el-form-item label="菜单图标" prop="icon"
						:rules="[{required: true, message: '请选择菜单图标', trigger: 'blur'}]">
						<e-icon-picker v-model="form.icon" placeholder="请选择菜单图标" size="small" clearable />
					</el-form-item>

					<el-form-item label="菜单配置" v-show="form.menuId=='' || form.isMenu==1">
						<el-checkbox v-model="form.isMenu" :true-label="1" :false-label="0" label="是否菜单"
							@change="menuChange" v-show="form.menuId==''">
						</el-checkbox>
						<el-checkbox v-model="form.isClose" :true-label="1" :false-label="0" label="是否可关闭"
							v-show="form.isMenu==1">
						</el-checkbox>
						<el-checkbox v-model="form.isExternal" v-show="form.isMenu==1" :true-label="1" :false-label="0"
							label="是否外部地址" @change="externalChange">
						</el-checkbox>
						<el-checkbox v-model="form.isHidden" :true-label="1" :false-label="0" label="是否隐藏"
							v-show="form.isMenu==1">
						</el-checkbox>
		...
		methods: {
			...
			onSubmit(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.loading = true;
						this.$ajax.post(this.getContentPath() + "/admin/system/menu/menu/saveOrUpdate", this.form).then(
							res => {
								this.loading = false;
